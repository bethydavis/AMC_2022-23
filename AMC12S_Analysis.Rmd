...
Filter according to the quality note + primer length parameters - post March 22, run the filtout12sync, not the individual trims
...
Starting 5/2, just do the merged (unless wanting to test)
...
B - Merged:
Since the merge has already happened, we don't require making a sequence table before removing chimeras
...
### NTS: This chunk is no longer working, as seqtab.nochim is in the form of a list. Did the structure of the output change (it used to work)? Confer on if the results need the verification, espec. after running COI
...
### NTS: review refdb and make sure the parameters are satisfactory, then proceed from here

# Assign Taxonomy
```{r assign_taxonomy, echo = FALSE}
# Very slow and intensive
all.taxa <- assignTaxonomy(seqtab12.nochim, 'C:/Users/bydav/Desktop/SpeciesList_Cleaning2024/a04_REFDBdada.fasta', tryRC = TRUE, verbose = TRUE)
beep(sound = "fanfare")
```
