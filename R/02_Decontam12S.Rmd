...
This .rmd will only finish processing for the 12S primer dataset. See 02_DecontamCOIB.Rmd to finish sample processing for the COIB primer dataset.
...
plot_bar(phy_ini.func, fill="Phylum") + ggtitle("Proportion by Phylum of Identified Reads - 12S")

# Save plot
ggsave("PreDecontam_IDProportions.png", path = path_figM, plot_bar(phy_ini.func, fill="Phylum") + ggtitle("Proportion by Phylum of Identified Reads"), width = 7, height = 4, units = "in") 
```
...
# Save the plot
ggsave("DecontamLibrarySize_Type.png", path = path_figM, ggplot(data=df_ini, aes(x=Index, y=LibrarySize, color=Type)) + geom_point() + ggtitle("12S Identified Read Numbers per Sample, by Sample Type"), width = 6, height = 4, units = "in") 
```
...
saveRDS(phy_trim, "Data/Outputs/2025-06-17/12S/InterimObjects/DecontamRemoved.rds")
```
...
# Save plots
ggsave("TrimmedPhyla.png", plot = trim_phyla, path = path_figM, width = 6, height = 4, units = "in")
ggsave("TrimmedClass.png", plot = trim_class, path = path_figM, width = 6, height = 4, units = "in")
```
...
saveRDS(clean_phy, "Data/Outputs/2025-06-17/12S/InterimObjects/CleanPhyloseq.RDS")
```
